...
export default function FormPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <form onSubmit={handleSubmit} className="w-full max-w-4xl bg-white rounded-lg shadow-lg p-6 flex">
        
        {/* Sidebar */}
        <div className="w-1/4 p-4 border-r border-gray-200">
          <h1 className="text-xl font-bold text-blue-600 mb-4">Research Analysis</h1>
          <p className="text-gray-600 mb-6">Analyze the research paper by answering the questions below.</p>
            ...
            {questions.map((_, index) => (
              <button
                ...
                className={`flex items-center px-3 py-2 rounded-lg text-sm text-left ${
                  currentQuestion === index ? 'bg-blue-100 text-blue-600 font-semibold' : 'text-gray-700 hover:bg-gray-100'
                }`}
              >
                Question {index + 1}
              </button>
        ...
        <div className="w-3/4 p-4">
          ...
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-500 mt-1">Progress: {Math.round(progress)}%</p>
          </div>

          {/* Name Field */}
          <div className="mb-6">
            <label className="block text-gray-700 font-semibold mb-2">Enter your full name:</label>
            <input
              type="text"
              value={name}
