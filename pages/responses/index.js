  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <div className="w-full max-w-4xl bg-white rounded-lg shadow-lg p-8 space-y-8">
        <h1 className="text-4xl font-extrabold text-blue-700 mb-6 text-center">All Responses</h1>
              ...
              <Link href={`/responses/${response._id}`}>
                <p className="block p-6 bg-blue-50 hover:bg-blue-100 rounded-lg shadow-sm border border-gray-200 transition-all duration-200">
                  <div className="flex justify-between items-center">
                    <p className="text-lg font-semibold text-blue-700">{response.name}</p>
                    <p className="text-sm text-gray-500">{new Date(response.createdAt).toLocaleDateString()}</p>
                  </div>
                </p>
              </Link>
            </li>
